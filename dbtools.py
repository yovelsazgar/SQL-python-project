...
class Dao(object):
    ...
    def updateQuantity(self, newQuantity, productId):
        self._conn.execute("""UPDATE products SET quantity=(?) WHERE id=(?)""",[newQuantity,productId])
    ...
    def getRecordsById(self):
        c = self._conn.cursor()
        c.execute('SELECT * FROM {} ORDER BY id'.format(self._table_name))
        return orm(c, self._dto_type)
